...
const ModalEmpresaRegistro = ({ setModalAlianza }) => {
    ...
    return (
        <div className=" bg-black bg-opacity-90 h-screen w-full absolute top-0 bottom-0 left-0 right-0">
            ...
            <FaRegTimesCircle
                className="text-white text-4xl m-4 cursor-pointer"
            ...
            <div className="w-1/2 mx-auto">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <input value={name} onChange={e => setName(e.target.value)} className=' mt-9 mb-2 p-2 rounded-2xl outline-none w-full' type="text" placeholder='Nombre empresa' />
                    <input value={rut} onChange={e => setRut(e.target.value)} className=' mt-9 mb-2 p-2 rounded-2xl outline-none w-full' type="number" placeholder='RUT de la empresa' />
                    <input value={correo} onChange={e => setCorreo(e.target.value)} className=' mt-9 mb-2 p-2 rounded-2xl outline-none w-full' type="email" placeholder='Correo' />
                    <input value={contacto} onChange={e => setContacto(e.target.value)} className=' mt-9 mb-2 p-2 rounded-2xl outline-none w-full' type="number" placeholder='Número de contacto' />
                    <input value={direccion} onChange={e => setdireccion(e.target.value)} className=' mt-9 mb-2 p-2 rounded-2xl outline-none w-full' type="string" placeholder='Direccion' />

                    <div className="mt-5">
                        <label htmlFor="horarios" className="text-white block mb-2">Horarios en las que el recuperador va por los residuos:</label>
                        <div className="md:flex gap-4">
                            <input value={horario1} onChange={e => sethorario1(e.target.value)} className='mb-2 p-2 rounded-2xl outline-none w-1/2' type="text" placeholder='Horario 1' />
                            <input value={horario2} onChange={e => sethorario2(e.target.value)} className='mb-2 p-2 rounded-2xl outline-none w-1/2' type="text" placeholder='Horario 2' />
                        </div>
                    </div>
